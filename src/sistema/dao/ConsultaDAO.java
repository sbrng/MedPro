...
public class ConsultaDAO {
	public boolean create(Consulta u) {
		...
		try {
			String dataHora = u.getDataHora().get(Calendar.HOUR_OF_DAY) + ":" + u.getDataHora().get(Calendar.MINUTE)
					+ ", " + u.getDataHora().get(Calendar.DAY_OF_MONTH) + "/" + u.getDataHora().get(Calendar.MONTH)
					+ "/" + u.getDataHora().get(Calendar.YEAR);

			stmt = con.prepareStatement(
					"INSERT INTO consultas (medicoId, pacienteId, dataHora, localidade, sintomas)VALUES(?,?,?,?,?)");
	...
	public void update(Consulta u) {
		...
		try {
			...
			stmt = con.prepareStatement(
					"UPDATE consultas SET medicoId = ?, pacienteId = ?, dataHora = ?, localidade = ?, sintomas = ? WHERE id = ?");
			stmt.setString(1, u.getMedicoId());
			stmt.setString(2, u.getPacienteId());
			stmt.setString(3, dataHora);
			stmt.setString(4, u.getLocal());
